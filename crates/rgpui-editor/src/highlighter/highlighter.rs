...
impl rgpui::util::sum_tree::Item for HighlightItem {
...
impl rgpui::util::sum_tree::Summary for HighlightSummary {
    type Context<'a> = &'a ();
...
impl<'a> rgpui::util::sum_tree::Dimension<'a, HighlightSummary> for usize {
    fn zero(_: &()) -> Self {
        0
    }
...
impl<'a> rgpui::util::sum_tree::Dimension<'a, HighlightSummary> for Range<usize> {
    fn zero(_: &()) -> Self {
        Default::default()
    }
...
mod tests {
    ...
    use super::*;
    use rgpui_component::Colorize as _;
        ...
        fn color_name(c: Option<Hsla>) -> String {
            match c {
                Some(c) => {
                    if c == rgpui::red() {
                        "red".to_string()
                    } else if c == rgpui::green() {
                        "green".to_string()
                    } else if c == rgpui::blue() {
                        "blue".to_string()
                    } else {
                        c.to_hex()
                    }
...
"#;
    ...
    fn test_unique_styles() {
        let red = color_style(rgpui::red());
        let green = color_style(rgpui::green());
        let blue = color_style(rgpui::blue());
